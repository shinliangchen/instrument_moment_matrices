% This code compute a DI lower bound on TSR given a temporal CHSH
...
% [S_num,~] = AMM_proj_seq_str2num(d,ny,nb,S);
% %AMM_new = AMM_str2num_blk_temporal(d,nx,S_num,'real'); % randomnly generate AMM, in the block-diagonal form
...
% while r > 10^-7
%     S_num = AMM_proj_seq_str2num(d,ny,nb,S);
%     %AMM_new = AMM_str2num_blk_temporal(d,nx,S_num,'real');
%     AMM_new = AMM_str2num_blk_temporal_ver2(d,nx,S_num,'real');
%     
%     %SL: randomnly generate AMM (in the block-diagonal form) until the set
...
%     Gnew = Gnew./r;
%     %disp(r);
%     G = [G,Gnew];%SL: set of orthonormal vectors (it forms an orthonormal basis when the loop is finished)
% end
% 
% sum_mu_Gk = 0;
% for kk=1:size(G,2)
%     Gk{kk} = reshape(G(:,kk),sqrt(length(G(:,1))),[]);% SL: reshape vectorized G into the the matrix form
%     %GG = GG + X(kk)*(Gk+Gk');
%     %mu{a,x,kk} = sdpvar(1,1,'hermitian','real');
%     %sum_mu_Gk{a,x} = sum_mu_Gk{a,x} + mu{a,x,kk}.*Gk{kk};
