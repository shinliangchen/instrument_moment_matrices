% self-testing of 2-->1 QRAC states [Fig.3]
clear all;
tic;
...
str_By1 = string('B_1|1*B_1|2');
if any(uni_mono==AMM_proj_adjoint_mono(str_By1))
    str_By1 = AMM_proj_adjoint_mono(str_By1);
elseif any(uni_mono==str_By1)==0
    error('something is wrong')
end

str_By2 = string('B_1|1*B_1|2*B_1|1');
if isempty(find(uni_mono==str_By2))
    error('the level of the hierarchy is not high enough to produce the monomial')
end


f00 = (s^2) + (c^2-s^2)*u{uni_mono==string('B_1|1'),1,1} - 4*c*s*u{uni_mono==str_By2,1,1} +...
    +2*c*s*u{uni_mono==str_By1,1,1} + 2*c*s*u{uni_mono==str_By1,1,1};

f10 = (c^2) + (s^2-c^2)*u{uni_mono==string('B_1|1'),2,1} - 4*c*s*u{uni_mono==str_By2,2,1} +...
    +2*c*s*u{uni_mono==str_By1,2,1} + 2*c*s*u{uni_mono==str_By1,2,1};

f01 = (s^2) + (c^2-s^2)*u{uni_mono==string('B_1|1'),1,2} + 4*c*s*u{uni_mono==str_By2,1,2} +...
    -2*c*s*u{uni_mono==str_By1,1,2} - 2*c*s*u{uni_mono==str_By1,1,2};

f11 = (c^2) + (s^2-c^2)*u{uni_mono==string('B_1|1'),2,2} + 4*c*s*u{uni_mono==str_By2,2,2} +...
    -2*c*s*u{uni_mono==str_By1,2,2} - 2*c*s*u{uni_mono==str_By1,2,2};

fidelity_all = (1/4).*(f00+f10+f01+f11);
...
constr = [constr, SE==value_QRAC];
%opts = sdpsettings('verbose', 0);
sol=solvesdp(constr , fidelity_all);%, opts);
double(fidelity_all)
sol
